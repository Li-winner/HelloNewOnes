...
3. 传统 CV 基础(走马观花即可，后面需要用到再学）
3. 线性代数（推荐: [入门](https://www.bilibili.com/video/BV1Ys411k7yQ/) [进阶](https://www.bilibili.com/video/BV16Z4y1U7oU/)）与Eigen库的使用 
4. 自定义训练集下 YOLO 训练及调参  
5. 相机模型、标定、2D-3D (PNP)  
6. Git的常用操作和github的简单使用
...
1. ROS2 基础
   - 推荐教程：
      ...
      【《ROS 2机器人开发从入门到实践》<https://b23.tv/mw4FOq3>】


   - （了解）TF2坐标转换、TF树等相关概念

   - 简单学习下foxglove可视化工具(可参考asserts下"foxglove简易教程.md")


3. python基础

4. 常用调试工具、设备认识和使用（可跳过）
...
4. 了解**全局路径规划**(A*,Dijkstra)等原理，了解**局部规划算法**（TEB,DWA )的原理（推荐：高飞 移动机器人运动规划）

5. 补充**里程计**相关知识，了解**定位方法**的原理
...
6. 尝试使用 Gammping 或 cartographer 使用激光雷达在仿真和真实世界中建图

7. 尝试 LIO 如 Fast-LIO 的 Demo 运行（Bag、真实设备）

8. 尝试 VIO 如(ORB-SLAM、VINS-FUSION)的 Demo 运行（Bag、真实设备）

9. 使用 Nav2 框架在真实世界进行 2d 下的机器人的导航规划（激光雷达使用 2d,里程计使用 LIO 或 VIO 输出或轮式里程计，预建全局地图）

10. 使用 Nav2 的决策树模块，模拟赛场哨兵决策。

#### 2.2.2 SLAM 进阶

1. 了解各种传感器（IMU（推荐:《自动驾驶与机器人中的SLAM技术》3讲），深度相机，雷达，UWB）等特性，以及其融合、标定、同步方案

2. 了解进阶滤波器，如: IEKF（知乎），ESKF（推荐:《自动驾驶与机器人中的SLAM技术》3讲），IESKF

3. 了解FAST LIO原理与实现: [paper](https://arxiv.org/abs/2010.08196) [code ](https://github.com/hku-mars/FAST_LIO)，faster lio和FAST LIO2的改进点可简单了解，同时关注更多定位算法

4. 了解far planner原理与实现: [paper](https://arxiv.org/abs/2110.09460) [code](https://github.com/MichaelFYang/far_planner)，同时关注更多规划算法
...
2. 深入学习算法原理（IMU 预积分，建图算法，规划算法，感知算法等）

3. 心有余力者，可多看书如《机器人的状态估计》，多看论文（Mars Lab(hku) , HKUST Aerial Robotics Group , Fast Lab (zju) , STAR Lab(sysu) ）
